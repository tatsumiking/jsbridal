...
function fncDispSitNameTable()
{
    ...
    clsTbl = m_clsLayout.tlines[m_taLeft.nLine].tables[m_taLeft.nClum];
    ...
    clsTbl = m_clsLayout.tlines[m_taRight.nLine].tables[m_taRight.nClum];
...
function fncSitGestName(sCid, nGestId)
{
    ...
    if(nGestId == 0){
        ...
        idx = fncGetGestSitIndex(nGestId);
        name = m_aryGestSit[idx].name;
        ary = name.split(" ");
        txtCnt.value = ary[0];
    }
}
